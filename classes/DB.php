  ...
  public function __construct()
  {
    try {
      ...
      */
      ...
      $this->_mysqli = mysqli_connect("localhost", 'root', "root", "investor");
  ...
  public static function fetchAllCount($conn){
 	...
 	public static function update($table, $data, $where){
 		...
 		foreach($where as $k=>$v){
 			if(is_numeric($k)){
 				die("Where clause cannot be integer; k: $k; value: $v");
 			}
 		...
 		if(empty($where_query)){
 			die('Where-query variable cannot be empty.');		
 		}
 		if(empty($data_query)){
 			die('Data-query variable cannot be empty.');		
 		}
